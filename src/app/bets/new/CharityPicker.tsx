...
export default function CharityPicker({
  ...
  if (charities.length === 0) {
    return <p className="text-sm text-zinc-500">Loading charities…</p>;
  }
    ...
    <div className="flex flex-col gap-2">
      <div className="grid grid-cols-2 gap-2">
        <select
          ...
          className="rounded-lg border border-flamingo-200 bg-white px-3 py-2 text-sm text-zinc-900"
        >
          {categories.map((cat) => (
      ...
      {selected && (
        <div className="flex items-center gap-3 rounded-lg bg-white/70 px-3 py-2">
          ...
          <p className="line-clamp-2 text-xs text-zinc-600">{selected.description}</p>
        </div>
      )}
      ...
      <a
        ...
        className="inline-flex w-fit items-center gap-1 text-xs font-medium text-flamingo-700 hover:underline"
      >
